...
-- Fungsi SHA256 menggunakan crypto bawaan executor (jika ada)
local function sha256(input)
    if crypto and crypto.sha256 then
        return crypto.sha256(input)
    else
        error("Vechnost Error: Executor tidak mendukung crypto.sha256. Gunakan executor lain (Synapse/Krnl/dll).")
    end
end
