-- vechnost_webhook.lua
-- Versi dengan key‑based authentication (tanpa whitelist ID Roblox)

local Rayfield = loadstring(game:HttpGet("https://sirius.menu/rayfield"))()

-- =====================================================
-- FUNGSI UNTUK MEMUAT DAFTAR KEY DARI GITHUB
-- =====================================================
local function loadKeys()
    local success, keys = pcall(function()
        return loadstring(game:HttpGet("https://raw.githubusercontent.com/allarsky/vechnost/refs/heads/main/keys.lua"))()
    end)
...
local Tab = Window:CreateTab("Activation", "key")
...
Tab:CreateButton({
    Name = "Activate",
    Callback = function()
        local key = keyInput.CurrentValue:gsub("%s+", "")
        if key == "" then
            Rayfield:Notify({ Title = "Vechnost", Content = "Key tidak boleh kosong!", Duration = 3 })
            return
        end

        local keys = loadKeys()
        if not keys then
            Rayfield:Notify({ Title = "Vechnost", Content = "Gagal memuat database key!", Duration = 3 })
            return
        end

        if keys[key] then
            Rayfield:Notify({ Title = "Vechnost", Content = "Key valid! Memuat notifier...", Duration = 2 })
            Window:Destroy()  -- tutup GUI activation
            StartNotifier()   -- jalankan fitur utama
        else
            Rayfield:Notify({ Title = "Vechnost", Content = "Key tidak valid!", Duration = 3 })
        end
    end
})
...
Tab:CreateParagraph({
    Title = "Info",
    Content = "Masukkan key yang kamu terima dari Discord.\nJika belum punya, redeem di panel bot."
})

-- =====================================================
-- FUNGSI UTAMA NOTIFIER (DIAMBIL DARI FILE LAMA)
-- =====================================================
function StartNotifier()
            ...
            components = {
                {
                    ...
                    components = {
                        ...
                        { type = 10, content = "@" .. (playerName or "Unknown") .. " you got new a " .. string.upper(rarityName) .. " fish" },
                        {
                            type = 9,
                            components = {
    ...
    -- =====================================================
    -- BAGIAN 11: RAYFIELD UI (Notifier Settings)
    -- =====================================================
    local Window = Rayfield:CreateWindow({
        Name = "Vechnost",
        Icon = "webhook",
...
-- Script akan menunggu di GUI activation hingga user memasukkan key yang valid.
-- Setelah valid, StartNotifier() dipanggil dan GUI activation ditutup.
