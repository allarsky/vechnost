...
local sha256 = (function()
    ...
    local function rightRotate(x, n) return (x >> n) | (x << (32 - n)) end
    ...
    return function(msg)
        ...
        for i = 1, 8 do
            bytes[#bytes + 1] = (bitLen >> (56 - (i - 1) * 8)) & 0xFF
        end
        ...
        for chunkStart = 1, #bytes, 64 do
            ...
            for i = 1, 16 do
                w[i] = 0
                for j = 1, 4 do
                    w[i] = (w[i] << 8) | bytes[chunkStart + (i - 1) * 4 + j - 1]
                end
            end
            for i = 17, 64 do
...
-- =====================================================
-- BAGIAN 0: AUTHENTICATION (WHITELIST DENGAN HASH)
-- =====================================================
local function CheckWhitelist()
    ...
    local hashedId = sha256(tostring(userId))

    if not whitelist[hashedId] then
        game.Players.LocalPlayer:Kick("Vechnost Access: Akun kamu (" .. userId .. ") belum terdaftar. Silahkan redeem key di discord.gg/vechnost")
        return false
    end
    return true
end
